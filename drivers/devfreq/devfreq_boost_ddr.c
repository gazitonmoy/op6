...
#define pr_fmt(fmt) "devfreq_boost_ddr: " fmt
...
static unsigned short flex_boost_ddr_duration __read_mostly = CONFIG_FLEX_DEVFREQ_BOOST_DDR_DURATION_MS;
static unsigned short input_boost_ddr_duration __read_mostly = CONFIG_DEVFREQ_INPUT_BOOST_DDR_DURATION_MS;
static unsigned int devfreq_thread_prio __read_mostly = CONFIG_DEVFREQ_DDR_THREAD_PRIORITY;
...
module_param(flex_boost_ddr_duration, short, 0644);
module_param(input_boost_ddr_duration, short, 0644);
...
struct df_boost_drv {
	struct boost_dev devices[DEVFREQ_MAX_DDR];
...
static void __devfreq_boost_ddr_kick(struct boost_dev *b)
{
	...
	if (!mod_delayed_work(b->wq_i, &b->input_unboost,
			msecs_to_jiffies(input_boost_ddr_duration))) {
			       ...
			       void *data)
{
	...
	for (i = 0; i < DEVFREQ_MAX_DDR; i++) {
		struct boost_dev *b = d->devices + i;

		if (*blank == MSM_DRM_BLANK_UNBLANK_CUST) {
			devfreq_boost_ddr_kick_wake(DEVFREQ_MSM_DDRBW, 1000);
...
static void devfreq_boost_ddr_input_event(struct input_handle *handle,
				      ...
				      int value)
{
	...
	for (i = 0; i < DEVFREQ_MAX_DDR; i++)
		__devfreq_boost_ddr_kick(d->devices + i);
}

static int devfreq_boost_ddr_input_connect(struct input_handler *handler,
				       ...
				       const struct input_device_id *id)
{
	...
	handle->name = "devfreq_boost_ddr_handle";
...
static void devfreq_boost_ddr_input_disconnect(struct input_handle *handle)
{
...
static const struct input_device_id devfreq_boost_ddr_ids[] = {
...
static struct input_handler devfreq_boost_ddr_input_handler = {
	.event		= devfreq_boost_ddr_input_event,
	.connect	= devfreq_boost_ddr_input_connect,
	.disconnect	= devfreq_boost_ddr_input_disconnect,
	.name		= "devfreq_boost_ddr_handler",
	.id_table	= devfreq_boost_ddr_ids
};

static int __init devfreq_boost_ddr_init(void)
{
	struct df_boost_drv *d = &df_boost_drv_g;
	struct task_struct *thread[DEVFREQ_MAX_DDR];
	int i, ret;

	for (i = 0; i < DEVFREQ_MAX_DDR; i++) {
		struct boost_dev *b = d->devices + i;
		b->wq_i = alloc_workqueue("devfreq_boost_ddr_wq_i", WQ_HIGHPRI, 0);
		b->wq_f = alloc_workqueue("devfreq_boost_ddr_wq_f", WQ_HIGHPRI, 0);
		b->wq_m = alloc_workqueue("devfreq_boost_ddr_wq_m", WQ_HIGHPRI, 0);
		
		thread[i] = kthread_run_low_power(devfreq_boost_thread, b,
						      "devfreq_boost_ddrd/%d", i);
		if (IS_ERR(thread[i])) {
	...
	devfreq_boost_ddr_input_handler.private = d;
	ret = input_register_handler(&devfreq_boost_ddr_input_handler);
	if (ret) {
...
late_initcall(devfreq_boost_ddr_init);
