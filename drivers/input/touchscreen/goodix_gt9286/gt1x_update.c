...
static int gt1x_search_update_files(void)
{
	...
	while (retry-- > 0) {
		...
		GTP_INFO("IS_ERR.%d",IS_ERR(pfile));
		if (IS_ERR(pfile)) {
			GTP_INFO("11.");
...
int gt1x_update_firmware(void *filename)
{
	...
	p = gt1x_get_fw_data(update_info.firmware_info->subsystem[0].offset,
...
_reset:
	...
	if ((fw_ver_info.patch_id & 0xFFFF) == (ver_info.patch_id & 0xFFFF)) {
		GTP_INFO("The version of the fw is same the IC's!");
		return ERROR_CHECK;
	}
	return 0;
}
